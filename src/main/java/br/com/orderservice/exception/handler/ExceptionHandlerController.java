...
public class ExceptionHandlerController {
    ...
    public ResponseEntity<Object> handleOrderNotExistsException() {
	...
	public Collection<ErrorResponse> handleMethodArgumentNotValidExceptionInContext(
			final MethodArgumentNotValidException ex) {
		final var errors = new ArrayList<ErrorResponse>();

		errors.addAll(ex.getBindingResult().getGlobalErrors().stream()
				.map(violation -> ErrorResponse.as(message(violation))).collect(Collectors.toList()));

		errors.addAll(ex.getBindingResult().getFieldErrors().stream()
				.map(violation -> ErrorResponse.as(message(violation)).tag(violation.getField()))
	...
	private ErrorResponse handleJsonMappingException(final JsonMappingException ex) {
		...
		return ErrorResponse.as(messageSource.getMessage("message.invalidParameterValue", new Object[] { paramName },
				LocaleContextHolder.getLocale())).tag(paramName);
	}
